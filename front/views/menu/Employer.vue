              ...
              <v-dialog v-model="dialog" max-width="500px">
                ...
                <v-card>
                  ...
                  <v-card-actions>
                    ...
                    <v-btn color="blue darken-1" text @click="save" :disabled='checkRegisterUserForSaveButton'>儲存</v-btn>
  ...
  computed: {
    ...
    checkRegisterUserForSaveButton() {
      if (!!this.editedItem.emp_id && !!this.editedItem.emp_name &&
          ...
          this.empIDErrMsg == '' && this.nameErrMsg == '') {
        return false;
      } else {
        return true
      }
  ...
  methods: {
    ...
    */
    ...
    removeUser(value) {
      ...
      axios.post(path, payload)
      .then(res => {
          console.log("remove user ok", res.data.status);
      })
      .catch(err => {
          console.error(err)
      });
    ...
    save () {
      ...
      if (this.editedIndex == -1) {    //add
        ...
        console.log("tosterOK: ", this.tosterOK);
        if (!this.tosterOK) {
